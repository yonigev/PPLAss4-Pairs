...
//new task 2 Type Inference
export type PairTExp = { tag: "PairTExp",car: NonTupleTExp, cdr:NonTupleTExp};
export const makePairTExp = (car: NonTupleTExp, cdr:NonTupleTExp): PairTExp => ({tag: "PairTExp", car: car, cdr:cdr});
...
//added task 2
export const pairTExpComponents = (p: PairTExp): TExp[]=>
    [p.car,p.cdr];
...
const parseCompoundTExp = (texps: any[]): ProcTExp | PairTExp | Error => {
    ...
    if(pair_pos === -1){
        //console.log("\n\nnot pair - " + JSON.stringify(texps,null,2));
...
export const unparseTExp = (te: TExp | Error): string | Error => {
    ...
    const up = (x: TExp | Error): string | string[] | Error =>
        ...
        isPairTExp(x) ? '(Pair '+unparseTExp((<PairTExp>te).car)+' '+unparseTExp((<PairTExp>te).cdr)+')':
        ["never"];
...
const matchTVarsInPairs = <T1, T2>(te1: TExp, te2: TExp,
    succ: (mapping: Array<Pair<TVar, TVar>>) => T1,
    fail: () => T2): T1 | T2 =>
